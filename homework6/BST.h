...
protected:
    ...
    void calculate_height(BinaryNode *&t) {
        if (t != nullptr)
            t -> height = std::max(height(t -> right), height(t -> left)) + 1;
    }

    int get_balancefactor (BinaryNode *&t) {
    ...
    void balance (BinaryNode *&t) {
        if (t != nullptr) {
            if (get_balancefactor(t) > 1) {
                if (get_balancefactor(t -> left) <= 0) {
                    BinaryNode *node1 = t -> left -> right;
                    t -> left -> right = node1 -> left;
                    node1 -> left = t -> left;
                    calculate_height(t -> left);
                    calculate_height(node1);
                    t-> left = node1;

                    BinaryNode *node2 = t -> left;
                    t -> left = node2 -> right;
                    node2 -> right = t;
                    calculate_height(t);
                    calculate_height(node2);
                    t = node2; 

                } else {
                    BinaryNode *node = t -> left;
                    t -> left = node -> right;
                    node -> right = t;
                    calculate_height(t);
            ...
            else if (get_balancefactor(t) < -1) {
                if (get_balancefactor(t -> right) >= 0) {
                    BinaryNode *node1 = t -> right -> left;
                    t -> right -> left = node1 -> right;
                    node1 -> right = t -> right;
                    calculate_height(t -> right);
                    calculate_height(node1);
                    t -> right = node1;

                    BinaryNode *node2 =  t -> right;
                    t ->right = node2 -> left;
                    node2 -> left = t;
                    calculate_height(t);
                    calculate_height(node2);
